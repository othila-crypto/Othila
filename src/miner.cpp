...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution.
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("OTHMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("OTHMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("OTHMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("othila-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running OTHMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
