...
#if defined(NDEBUG)
#error "OTH cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uothila) are considered zero fee (for relaying and mining)
...
{
    RenameThread("othila-scriptch");
    scriptcheckqueue.Thread();
}

bool RecalculateOTHSupply(int nHeightStart)
{
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // OTH: We use certain sporks during IBD, so check to see if they are
