...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::OTH);
    ...
    if (!settings.contains("nAnonymizeOthilaAmount"))
        settings.setValue("nAnonymizeOthilaAmount", 1000);

    nAnonymizeOthilaAmount = settings.value("nAnonymizeOthilaAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeOthilaAmount"))
        SoftSetArg("-anonymizeothilaamount", settings.value("nAnonymizeOthilaAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in othila.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeOthilaAmount:
            return QVariant(nAnonymizeOthilaAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeOthilaAmount:
            nAnonymizeOthilaAmount = value.toInt();
            settings.setValue("nAnonymizeOthilaAmount", nAnonymizeOthilaAmount);
            emit anonymizeOthilaAmountChanged(nAnonymizeOthilaAmount);
            break;
        case CoinControlFeatures:
